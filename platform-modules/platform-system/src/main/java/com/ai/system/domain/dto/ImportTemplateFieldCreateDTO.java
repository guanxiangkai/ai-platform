 ...
 * 导入模板字段映射创建DTO
 * <p>
 * 用于创建接口入参，包含参数校验；更新接口使用 {@link ImportTemplateFieldDTO}
 * </p>
 *
 * @author guanxiangkai
 * @since 1.0.0
 */
@Schema(description = "导入模板字段映射创建DTO")
@AutoMapper(target = ImportTemplateField.class)
public record ImportTemplateFieldCreateDTO(
        ...
        @Schema(description = "目标字段", requiredMode = Schema.RequiredMode.REQUIRED) @NotBlank(message = "字段不能为空") @Size(max = 128, message = "字段长度不能超过128个字符") String field,
        ...
        @Schema(description = "是否完全匹配", requiredMode = Schema.RequiredMode.REQUIRED) @NotNull(message = "是否完全匹配不能为空") Boolean exactMatch,
        @Schema(description = "是否必填字段", requiredMode = Schema.RequiredMode.REQUIRED) @NotNull(message = "是否必填字段不能为空") Boolean required,
        @Schema(description = "是否多值字段", requiredMode = Schema.RequiredMode.REQUIRED) @NotNull(message = "是否多值字段不能为空") Boolean multiple,
